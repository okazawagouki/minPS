...
figid = gcf;
imf = max(1,figid.Number-1); snrf = imf+1;
figure(imf);  clf
...
if cmask(3),
  ...
  for clr = 1:Nclr,
        ...
        LPRcross(:,:,clr) = [vectorize(tmp),...
              vectorize(shift(tmp,[0 2])), vectorize(shift(tmp,[0 -2])), ...
              vectorize(shift(tmp,[2 0])), vectorize(shift(tmp,[-2 0]))];
  end  % clr
  ...
  for clr = 1:Nclr,
        LPRcross(:,:,clr) = [LPRcross(:,1,clr),...
                vectorize(shift(reshape(LPRcross(:,2,clr),Nly,Nlx),[0 -2])),...
                vectorize(shift(reshape(LPRcross(:,3,clr),Nly,Nlx),[0 2])), ...
                vectorize(shift(reshape(LPRcross(:,4,clr),Nly,Nlx),[-2 0])),...
                vectorize(shift(reshape(LPRcross(:,5,clr),Nly,Nlx),[2 0]))];
        aux = mean(LPRcross(:,:,clr),2);
        aux = reshape(aux,Nly,Nlx);
        pyr(indices,clr) = 4*vectorize(real(shrink(aux,2)));
  end  % clr
    ...
    %% Interpolate parents
      if (nsc<Nsc)
        for nor = 1:Nor
          ...
          rparents(:,nor,clr) = vectorize(real(tmp));
          rparents(:,Nor+nor,clr) = vectorize(imag(tmp));

          tmp = abs(tmp);
          parents(:,nor,clr) = vectorize(tmp - mean2(tmp));
        end
      else
        tmp = real(expand(pyrLow(pyr(:,clr),pind),2))/4;
        rparents(:,1:5,clr) = [vectorize(tmp),...
                  vectorize(shift(tmp,[0 2])), vectorize(shift(tmp,[0 -2])), ...
                  vectorize(shift(tmp,[2 0])), vectorize(shift(tmp,[-2 0]))];
        parents = [];
      end
...
if cmask(3),
    ...
    for clr = 1:Nclr,
      ...
      if (var2(imag(cou))/var2(real(cou)) > 1e-3)
        ...
        cou = real(cou);
        apyr(cousinInd,clr) = vectorize(cou);
      end
      ...
      for nor = 1:Nor,
        ...
        ch = real(ch);
        apyr(pyrBandIndices(pind,nband),clr) = vectorize(ch);
        %% Impose magnitude:
...
if  cmask(4),
      if (var2(imag(rcousins(:,:,clr)))/var2(real(rcousins(:,:,clr))) > 1e-3)
        ...
        pyr(cousinInd,clr) = vectorize(real(rcousins(:,:,clr)));
end % cmask(4)
